    ...
    try:
        ...
        async with TelegramClient(session_file, api_id, api_hash) as client:
            ...
            if not messages or not messages[0].text:
                await offf(update, context, user_id, reason="No valid messages found in Saved Messages ❌")
                return
            ...
            while forwarding_on:
                current_group = user_groups[group_index % total_groups]

                try:
                    # Process group link
                    if current_group.startswith('-') and current_group[1:].isdigit():
                        to_peer = int(current_group)
                        target_group = await client.get_entity(to_peer)
                        topic_id = None
                    else:
                    ...
                    # Log error and skip to next group
                    print(f"⚠️ Error forwarding to group {current_group}: {e}")
                    if update and update.message:
                        await update.message.reply_text(
                            f"⚠️ Error forwarding message:\n\n📎 Group: `{current_group}`\n🔴 Error: `{e}`",
                            parse_mode="Markdown"
                        )
        ...
        await offf(update, context, user_id, reason=f"An error occurred: {e}")
