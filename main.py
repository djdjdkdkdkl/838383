...
async def off(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Attempt to disconnect client if session exists
    if os.path.exists(session_file):
        api_id = user_data.get("api_id", "")
        api_hash = user_data.get("api_hash", "")
        client = TelegramClient(session_file, api_id, api_hash)
        try:
            # Attempt to disconnect without reconnecting
            await client.disconnect()
            print(f"Client disconnected for user {user_id}")
        except Exception as e:
            print(f"Error disconnecting client for user {user_id}: {e}")

    # Update forwarding status in config
    if "forwarding_on" in user_data and user_data["forwarding_on"]:
        user_data["forwarding_on"] = False
        ...
        if scheduler.running:
            for job in scheduler.get_jobs():
                if job.args[2] == user_id:  # Match user ID in job args
                    scheduler.remove_job(job.id)
                    job_removed = True
                    break
        ...
        # Stop scheduler if no jobs remain
...
async def forward_messages(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: str) -> None:
    try:
        ...
        # Validate forwarding status and data
        ...
        if post_index >= len(post_messages):
            post_index = 0
        group_index = user_data.get("group_index", 0)

        # Forward message to current group
        async with TelegramClient(session_file, api_id, api_hash) as client:
            
            try:
                current_post = post_messages[post_index]
                current_group = user_groups[group_index % len(user_groups)]
                # Process group link
                if current_group.startswith("https://t.me/"):
                    to_peer, topic_id = extract_group_and_topic_id(current_group)
                elif current_group.startswith('-') and current_group[1:].isdigit():
                    to_peer = int(current_group)
                    topic_id = None
                else:
                    print(f"Invalid group link: {current_group}")
                    raise ValueError(f"Invalid group link: {current_group}")

                # Process message
                if current_post.startswith("https://t.me/"):
                    from_peer, message_id = extract_chat_and_message_id(current_post)

                    if "t.me/+" in current_group:
                        target_group = await client(functions.messages.CheckChatInviteRequest(
                            hash=current_group.split('+')[1]
                        ))
                    ...
                    if from_peer and message_id:
                        ...
                        print(f"Message forwarded to group {current_group}.")
                    else:
                        print(f"Invalid Telegram message link: {current_post}")
                        raise ValueError(f"Invalid Telegram message link: {current_post}")
                else:
                    if "t.me/+" in current_group:
                        target_group = await client(functions.messages.CheckChatInviteRequest(
                            hash=current_group.split('+')[1]
                    ...
                    print(f"Message sent to group {current_group}.")

                # Update indices
                group_index = (group_index + 1) % len(user_groups)
                if group_index == 0:
                    post_index = (post_index + 1) % len(post_messages)
                ...
                error = f"⚠️ Error forwarding message to {current_group}\n\n🔴 Error: {e}"
...
async def forward_saved(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: str) -> None:
    try:
        ...
        async with TelegramClient(f'{user_id}.session', api_id, api_hash) as client:
            ...
            # Process only the current group based on the index
            group_link = user_groups[group_index]
            ...
            # Update the group index for the next interval
            group_index = (group_index + 1) % len(user_groups)
            user_data["group_index"] = group_index
            config_data["users"][user_id] = user_data
            with open("config.json", "w") as f:
        ...
        print(f"Message forwarding completed for group {group_link}. Waiting for next interval.")
        await asyncio.sleep(interval)
...
def main():
    ...
    application.add_handler(CommandHandler("time", timer))
